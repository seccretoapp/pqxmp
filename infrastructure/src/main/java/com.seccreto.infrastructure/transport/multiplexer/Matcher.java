...
public class Matcher {
  ...
  public static MatcherInterface http1Fast(String... extMethods) {
    String[] defaultMethods = { "OPTIONS", "GET", "HEAD", "POST", "PUT", "DELETE", "TRACE", "CONNECT" };
    if (extMethods.length == 0) {
      return prefixMatcher(defaultMethods);
    }
  ...
  public static MatcherInterface http1() {
    return input -> {
      try (BufferedReader reader = new BufferedReader(new InputStreamReader(new BufferedInputStream(input)))) {
        ...
        return parts.length > 2 && parts[2].startsWith("HTTP/1.");
      } catch (IOException e) {
  ...
  public static MatcherInterface http2() {
    return Matcher::hasHTTP2Preface;
  }
